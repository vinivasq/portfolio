...
const validate = (input) => {
    if (input.validity.valid) {
        ...
        input.parentElement.querySelector('.contact__form__input-error').innerHTML = showErrorMessage(input)
    }
}
...
messageInput.addEventListener('blur', () => {
    ...
    if(messageInput.value === '') {
        messageInput.parentElement.querySelector('.contact__form__input-error').innerHTML = `<i class="fa-solid fa-circle-info"></i>
        O campo de mensagem não pode ficar vazio.
        `
    }
    else {
        messageInput.parentElement.querySelector('.contact__form__input-error').innerHTML = ''
    }
})
